...
const tableList = options => {
  ...
  const headers = ['维度'].concat(new Array(column).fill().map((v, i) => `Class ${i + 1}`))
  const lists = new Array(row).fill().map((v, i) => [`Item ${i + 1}`].concat(numbers.slice(i * column, (i + 1) * column)))
